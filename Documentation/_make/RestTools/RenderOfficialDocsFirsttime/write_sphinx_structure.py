...
def findRstFiles(srcdir, startpos):
    result = []
...
def getTocTreeEntries(relpath2, f2name, sublevels, listOfOldNew):
    result = []
...
def buildLookUpTable1(rstfiles, capitalize):
    D = {}
...
def makeListOfOldNew(D, capitalize):
    listOfOldNew =[]
    keys = sorted(D.keys())
    for k in keys:
        f1name = k + D[k][0] + '.rst'
...
def main(srcdir, destdir, srcdirimages, verbose=None, capitalize=True):

    NL = '\n'
    srcdirlen = len(srcdir)
    fnameimages = 'Images.txt'

    rstfiles = findRstFiles(srcdir, srcdirlen+1)
    D = buildLookUpTable1(rstfiles, capitalize)
    ...
    listOfOldNew = makeListOfOldNew(D, capitalize)
    ...
    if 1 and 'copy':
        ...
        for f1relpath, relpath2, f2name, sublevels in listOfOldNew:
            ...
            tte = getTocTreeEntries(relpath2, f2name, sublevels, listOfOldNew)
            if tte:
                s = (
                    '.. toctree::\n'
                    '   :maxdepth: 5\n'
